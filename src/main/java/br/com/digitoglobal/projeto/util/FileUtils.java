...
//		fc.addChoosableFileFilter(new XMLFilter());
 ...
 // Returns the contents of the file in a byte array.
    ...
    public static void delete(String path) {
        if (path == null || path.isEmpty()) throw new RuntimeException("Empty Path");
        delete(new File(path), false);
    }

    public static boolean deleteQuietly(String path) {
        if (path == null || path.isEmpty()) return false;
        return delete(new File(path), true);
    }

    public static boolean delete(File file, boolean quietly) {
        if (file == null) {
